...
func UnaryClientInterceptor(opts ...Option) grpc.UnaryClientInterceptor {
	...
	return func(ctx context.Context, method string, req, reply interface{}, cc *grpc.ClientConn, invoker grpc.UnaryInvoker, callOpts ...grpc.CallOption) error {
		...
		tracer := cfg.tracerServer.TracerProvider.Tracer(cfg.tracerName, trace.WithInstrumentationVersion(SemVersion()))
...
func StreamClientInterceptor(opts ...Option) grpc.StreamClientInterceptor {
	...
	return func(ctx context.Context, desc *grpc.StreamDesc, cc *grpc.ClientConn, method string, streamer grpc.Streamer, callOpts ...grpc.CallOption) (grpc.ClientStream, error) {
		...
		isOk := cfg.checkLabel(method, cfg.excludeRegexMethod)
		if !isOk {
			return streamer(ctx, desc, cc, method, callOpts...)
		}
		requestMetadata, _ := metadata.FromOutgoingContext(ctx)
...
func UnaryServerInterceptor(opts ...Option) grpc.UnaryServerInterceptor {
	...
	return func(ctx context.Context, req interface{}, info *grpc.UnaryServerInfo, handler grpc.UnaryHandler) (interface{}, error) {
		...
		isOk := cfg.checkLabel(info.FullMethod, cfg.excludeRegexMethod)
		if !isOk {
			return handler(ctx, req)
		}
		...
		ctx, span := tracer.Start(trace.ContextWithRemoteSpanContext(ctx, spanCtx), name, trace.WithSpanKind(trace.SpanKindServer), trace.WithAttributes(attr...))
		if !span.IsRecording() {
			return handler(ctx, req)
		}
		defer span.End()
