...
func getValidPFuncs(reflectType reflect.Type) (vfs []ValidFunc, err error) {
	if reflectType.Kind() != reflect.Struct {
		return
	}
	vfss := make([]ValidFunc, 0)
	for i := 0; i < reflectType.NumField(); i++ {
		tagJson := reflectType.Field(i).Tag.Get("json")
		if tagJson == "" {
			vfs, err = getValidPFuncs(reflectType.Field(i).Type)
			for _, v := range vfs {
				vfss = append(vfss, v)
			}
			continue
		...
		if vfs, tag, err = getRegFuncs(tag, reflectType.Field(i).Name); err != nil {
			vfs = vfss
			return
		}
		fs := strings.Split(tag, ";")
		for _, vfunc := range fs {
			...
			vf, err = parseFunc(vfunc, reflectType.Field(i).Name, label)
			if err != nil {
				vfs = vfss
				return
			}
