...
// MinLength Test that the obj is same length to n if type is string or slice
func (v *Validation) MinLength(obj interface{}, n int, key string) *Result {
	return v.apply(MinLength{n, key}, obj)
}

// MaxLength Test that the obj is same length to n if type is string or slice
func (v *Validation) MaxLength(obj interface{}, n int, key string) *Result {
	return v.apply(MaxLength{n, key}, obj)
}
...
func (v *Validation) Valid(obj interface{}) (b bool, err error) {
	...
	for i := 0; i < objT.NumField(); i++ {
		...
		if tag == "" {
			if vfs, err = getValidPFuncs(objT.Field(i).Type); err != nil {
				return
			}
		} else {
