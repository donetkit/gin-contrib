...
func main() {
	...
	defer span.End()
	var num []int
	if err := dbs["default"].WithContext(ctx).Raw("SELECT holiday FROM calendar").Scan(&num).Error; err != nil {
		panic(err)
	}

	var str []string
	if err := dbs["default"].WithContext(ctx).Raw("SELECT id FROM school").Scan(&str).Error; err != nil {
		panic(err)
	}

	if err := dbs["default"].WithContext(ctx).Raw("SELECT id FROM school_air_conditioner_room").Scan(&str).Error; err != nil {
		panic(err)
	}
